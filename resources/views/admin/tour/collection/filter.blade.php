...
<div class="box">
    <div class="box-header">
        <h3 class="box-title" style="float:right;">
            <a href="{{ url('admin/tour/'.$collection->id .'/filter/create/')}}"><button class="btn btn-info" type="button"><span class="fa fa-plus"></span></button></a>
            <a href=""><button class="btn btn-default" type="button"><span class="fa fa-refresh"></span></button></a>
        </h3>
    </div>
    <div class="box-body table-responsive">
        {{ Form::open(array('url' => array('admin/tour/filter/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
        <table id="example4" class="table table-bordered table-striped">
            ...
            @foreach ($filter as $list)
                ...
                <tr class="ui-state-default" id="arrayorder_{{$list['id']}}">
                    <td align="center">
                        <a href="{{ url('admin/tour/'.$collection->id.'/filter/'.$list->id.'/edit')}}"><button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a>
                    </td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <a href="{{ url('admin/tour/'.$collection->id.'/filter/'.$list->id.'/destroy') }}" data-method="delete" name="delete_item">
...
<script>
    ...
    $( function() {
        $( "#sortable" ).sortable({opacity: 0.9,cursor: 'move', update: function() {
            ...
            //alert(order);
            $.post("{{url('api/cms/filter/reorder')}}", order, function(theResponse) {
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/tour/collection/{cid}/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/tour/collection/{cid}/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
