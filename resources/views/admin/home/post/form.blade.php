...
<div class="form-group{{ $errors->has('language') ? ' has-error' : '' }}">
    ...
    <div class="col-sm-5">
        {!! Form::select('language[]',\App\Homepost::$language, !empty($modes_selected)?$modes_selected:null, ['class' => 'select2 select2-hidden-accessible form-control', 'style' => 'width: 100%','multiple']) !!}
...
<div class="form-group{{ $errors->has('image_position') ? ' has-error' : '' }}">
    <label class="col-sm-2 control-label" for="image_position">Image_Position <span class="text-red">*</span></label>

    <div class="col-sm-5">

        @foreach (\App\Homepost::$position as $key => $value)
            <label>
                {!! Form::radio('image_position', $key, null, ['class' => 'flat-red']) !!} <span style="margin-right: 10px">{{ $value }}</span>
            </label>
        @endforeach

        @if ($errors->has('image_position'))
            <span class="help-block">
             <strong>{{ $errors->first('image_position') }}</strong>
            </span>
        @endif
    </div>
</div>
...
<div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
    ...
    <div class="col-sm-5">
        ...
        @foreach (\App\Homepost::$status as $key => $value)
            <label>
