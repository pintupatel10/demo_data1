...
<div class="box">
    <div class="box-header">
        <h3 class="box-title" style="float:right;">
            <a href="{{ url('admin/hotel/'.$collection->id .'/filter/create/')}}">
    ...
    <div class="box-body">
        {{ Form::open(array('url' => array('admin/hotel/filter/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
        <table id="example1" class="table table-bordered table-striped">
            ...
            @foreach ($filter as $list)
                <tr class="ui-state-default" id="arrayorder_{{$list['id']}}">
                    <td align="center">
                        <a href="{{ url('admin/hotel/'.$collection->id.'/filter/'.$list->id.'/edit')}}">
                            <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button>
                        </a>
                        {{--  <a href="{{ route('admin/hotel/'.$detail->id.'/checkpoint/edit/'.$list->id) }}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a> --}}
                    </td>

                    <td>{{ $list['name'] }}</td>
                    <?php
                        if(empty($list['hotel_list'])){
                            $p2=0;
                        }
                        else{
                            $p1 = explode(',', $list['hotel_list']);
                            $p2=count($p1);
                        }
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <a href="{{ url('admin/hotel/'.$collection->id.'/filter/'.$list->id.'/destroy') }}"
                                  name="delete_item">
...
<script>
    ...
    $( function() {
        $( "#sortable" ).sortable({opacity: 0.9,cursor: 'move', update: function() {
            ...
            //alert(order);
            $.post("{{url('api/cms/filter3/reorder')}}", order, function(theResponse) {
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/hotel/collection/{cid}/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/hotel/collection/{cid}/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
