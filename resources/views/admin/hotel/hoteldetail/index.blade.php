...
@section('content')
            ...
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title" style="float:right;">
                        <a href="{{ url('admin/hotel/hoteldetail/create/') }}" ><button class="btn btn-info" type="button"><span class="fa fa-plus"></span></button></a>
                        <a href="{{ url('admin/hotel/hoteldetail') }}" ><button class="btn btn-default" type="button"><span class="fa fa-refresh"></span></button></a>
                    </h3>
                </div>
                <!-- /.box-header -->

                <div class="box-body table-responsive">
                    {{ Form::open(array('url' => array('admin/hotel/hoteldetail/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
                    <table id="example2" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        <?php $count = 0;?>
                        @foreach ($hoteldetail as $list)
                            <?php $count++; ?>
                            <input type="hidden" name="pid<?php echo $count;?>" id="pid<?php echo $count;?>" value="{{ $list['id'] }}" />
                            <tr>
                                <td align="center">
                                    <a href="{{ route('admin.hotel.hoteldetail.edit', $list->id) }}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a>
                                </td>
                                <td>{{ $list['id'] }}</td>
                                <td>{{ $list['title'] }}</td>
                                <td>{{ $list['language'] }}</td>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title">Delete Hotel List</h4>
                                        </div>
                                        <div class="modal-body">
                                            <p>Are you sure you want to delete this Hotel List ?</p>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                                            <a href="{{ route('admin.hotel.hoteldetail.destroy',$list['id']) }}" data-method="delete" name="delete_item">
    ...
    {{-- <div style="text-align:right;float:right;"> @include('admin.pagination.limit_links', ['paginator' => $hoteldetail])</div>--}}
...
l.start();
$.ajax({
url: '{{url('api/cms/hotel/hoteldetail/assign')}}',
type: "get",
...
l.start();
$.ajax({
url: '{{url('api/cms/hotel/hoteldetail/unassign')}}',
type: "get",
data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
success: function(data){
    l.stop();
