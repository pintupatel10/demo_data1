...
<div class="box">
    <div class="box-header">
        <h3 class="box-title" style="float:right;">
            <a href="{{ url('admin/ticket/'.$collection->id .'/filter/create/')}}">
    ...
    <div class="box-body table-responsive">
        {{ Form::open(array('url' => array('admin/ticket/filter/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
        <table id="example4" class="table table-bordered table-striped">
            ...
            @foreach ($filter as $list)
                <tr class="ui-state-default" id="arrayorder_{{$list['id']}}">
                    <td align="center">
                        <a href="{{ url('admin/ticket/'.$collection->id.'/filter/'.$list->id.'/edit')}}">
                            <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button>
                        </a>
                        {{--  <a href="{{ route('admin/ticket/'.$detail->id.'/checkpoint/edit/'.$list->id) }}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a> --}}
                    </td>

                    <td>{{ $list['name'] }}</td>
                    <?php
                    if(empty($list['ticket_list'])){
                        $p2=0;
                    }
                    else{
                        $p1 = explode(',', $list['ticket_list']);
                        $p2=count($p1);
                    }
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <a href="{{ url('admin/ticket/'.$collection->id.'/filter/'.$list->id.'/destroy') }}"
                                    name="delete_item">
...
<script>
    ...
    $( function() {
        $( "#sortable" ).sortable({opacity: 0.9,cursor: 'move', update: function() {
            ...
            //alert(order);
            $.post("{{url('api/cms/filter1/reorder')}}", order, function(theResponse) {
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/ticket/collection/{cid}/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/ticket/collection/{cid}/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
