...
<div class="box">
    <div class="box-header" >
        <h3 class="box-title" style="float:right;">
            <a href="{{ url('admin/ticket/'.$detail->id .'/pricegroup/create/')}}" ><button class="btn btn-info" type="button"><span class="fa fa-plus"></span></button></a>
            <a href="{{ url('admin/ticket/ticketlist/'.$detail->id.'/edit') }}" ><button class="btn btn-default" type="button"><span class="fa fa-refresh"></span></button></a>
        </h3>
    </div>
    <div class="box-body">
        {{ Form::open(array('url' => array('admin/ticket/checkpoint/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
        <table id="example2" class="table table-bordered table-striped">
            ...
            @foreach ($pricegroup as $list)
                ...
                <tr>
                    <td align="center">
                        <a href="{{ url('admin/ticket/'.$detail->id.'/pricegroup/'.$list->id.'/edit')}}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a>
                    </td>

                    <td>{{ $list['id'] }}</td>
                    <td>{{ $list['title'] }}</td>
                    <?php $emailcout=App\TicketPrice::where('pricegroupid',$list['id'])->count()  ;?>
                    <td>{{$emailcout}}</td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title">Delete Ticket Pricegroup</h4>
                            </div>
                            <div class="modal-body">
                                <p>Are you sure you want to delete this Ticket Pricegroup ?</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                                <a href="{{ url('admin/ticket/'.$detail->id.'/pricegroup/'.$list->id.'/destroy') }}"  name="delete_item">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/ticket/ticketlist/{detailid}/pricegroup/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/ticket/ticketlist/{detailid}/pricegroup/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
