            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-info">
                        <div class="box-header with-border">
                            <h3 class="box-title">ADD Transportation Pricegroup </h3>
                        </div>
                        {!! Form::open(['url' => url('admin/transportation/'.$detail.'/pricegroup'), 'class' => 'form-horizontal','files'=>true]) !!}
                        <div class="box-body">
                ...
                <strong>{{ $errors->first('description') }}</strong>
                            ...
                            <div class="form-group{{ $errors->has('charge') ? ' has-error' : '' }}">
                                <label class="col-sm-2 control-label" for="charge">Service Charge <span class="text-red">*</span></label>
                                <div class="col-sm-5">
                                    {!! Form::text('charge', null, ['class' => 'form-control', 'placeholder' => 'Service Charge']) !!}
                ...
                <strong>{{ $errors->first('charge') }}</strong>
                            ...
                            <div class="form-group{{ $errors->has('time_table') ? ' has-error' : '' }}">
                                <label class="col-sm-2 control-label" for="time_table">Time Table </label>

                                <div class="col-sm-8">

                                    @foreach (\App\TransportationPricegroup::$time as $key => $value)
                                        <label>
                                            {!! Form::radio('time_table', $key, null, ['class' => 'flat-red']) !!} <span style="margin-right: 10px">{{ $value }}</span>
                                        </label>
                                    @endforeach

                                    @if ($errors->has('time_table'))
                                        <span class="help-block">
             <strong>{{ $errors->first('time_table') }}</strong>
            </span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
                                ...
                                <div class="col-sm-5">
                                    ...
                                    @foreach (\App\TransportationPricegroup::$status as $key => $value)
                                        <label>
