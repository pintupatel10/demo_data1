            ...
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title" style="float:right;">
                        <a href="{{ url('admin/transportation/transportationgroup/create/') }}" ><button class="btn btn-info" type="button"><span class="fa fa-plus"></span></button></a>
                        <a href="{{ url('admin/transportation/transportationgroup/') }}" ><button class="btn btn-default" type="button"><span class="fa fa-refresh"></span></button></a>
                    </h3>
                </div>
                ...
                <div class="box-body table-responsive">
                    {{ Form::open(array('url' => array('admin/transportation/transportationgroup/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                        <?php $count = 0;?>
                        @foreach ($groupdetail as $list)
                            <?php $count++; ?>
                            <input type="hidden" name="pid<?php echo $count;?>" id="pid<?php echo $count;?>" value="{{ $list['id'] }}" />
                            <tr>
                                <td align="center">
                                    <a href="{{ route('admin.transportation.transportationgroup.edit', $list->id) }}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a>
                                </td>
                                <td>{{ $list['id'] }}</td>
                                <td>{{ $list['title'] }}</td>
                                <td>{{ $list['language'] }}</td>
                                <?php $p1=explode(',',$list['transportation_list']);
                                $p2=array_filter($p1); ?>
                                <td>{{ count($p2) }}</td>
                                <td>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title">Delete Transportation Group</h4>
                                        </div>
                                        <div class="modal-body">
                                            <p>Are you sure you want to delete this Transportation Group ?</p>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                                            <a href="{{ route('admin.transportation.transportationgroup.destroy',$list['id']) }}" data-method="delete" name="delete_item">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/transportation/transportationgroup/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/transportation/transportationgroup/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
