<br><br>
<h1>Private Tour Price Table </h1>
<br>
<div class="box">
    <div class="box-header" >
        <h3 class="box-title" style="float:right;">
            <a href="{{ url('admin/privatetour/'.$detail."/".$pricegroup->id.'/price/create/')}}" ><button class="btn btn-info" type="button"><span class="fa fa-plus"></span></button></a>
            <a href="" ><button class="btn btn-default" type="button"><span class="fa fa-refresh"></span></button></a>
        </h3>
    </div>
    <div class="box-body">
        {{ Form::open(array('url' => array('admin/privatetour/price/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
            <?php $count = 0;?>
            @foreach ($tourprice as $list)
                <input type="hidden" name="pid<?php echo $count;?>" id="pid<?php echo $count;?>" value="{{ $list['id'] }}" />
                <tr>
                    <td align="center">
                        <a href="{{ url('admin/privatetour/'.$detail."/".$pricegroup->id.'/price/'.$list->id.'/edit')}}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a>
                    </td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title">Delete Private Tour Price</h4>
                            </div>
                            <div class="modal-body">
                                <p>Are you sure you want to delete this Private Tour Price ?</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                                <a href="{{ url('admin/privatetour/'.$detail."/".$pricegroup->id.'/price/'.$list->id.'/destroy') }}" name="delete_item">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/tour/tourlist/{detailid}/pricegroup/{pricegroupid}/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/tour/tourlist/{detailid}/pricegroup/{pricegroupid}/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
