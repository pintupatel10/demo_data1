...
<hr>
<h1>Private Tour Checkpoint </h1>
<br>
<div class="box">
    <div class="box-header" >

        <h3 class="box-title" style="float:right;">
            {{ Form::open(['url' => url('admin/privatetour/'.$detail->id.'/checkpoint'), 'id'=>'checkpoint_form','class' => 'form-horizontal','files'=>true]) }}
            ...
            <a href="{{ url('admin/privatetour/'.$detail->id .'/checkpoint/create/')}}" ><button class="btn btn-info" type="button"><span class="fa fa-plus"></span></button></a>
            <a href="{{ url('admin/privatetour/privatetourlist/'.$detail->id.'/edit') }}" ><button class="btn btn-default" type="button"><span class="fa fa-refresh"></span></button></a>
            {{ Form::close() }}
    ...
    <div class="box-body">
        {{ Form::open(array('url' => array('admin/privatetour/checkpoint/update_display_order'), 'method' => 'post','style'=>'display:inline')) }}
        <table id="example1" class="table table-bordered table-striped">
            <thead>
            <tr>
                <th>Edit</th>
            ...
            @foreach ($checkpoint as $list)
                <tr class="ui-state-default" id="arrayorder_{{$list['id']}}">
                    <td align="center">
                        <a href="{{ url('admin/privatetour/'.$detail->id.'/checkpoint/'.$list->id.'/edit')}}"> <button class="btn btn-info" type="button"><i class="fa fa-edit"></i></button></a>
                    </td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title">Delete Private Tour Checkpoint</h4>
                            </div>
                            <div class="modal-body">
                                <p>Are you sure you want to delete this Private Tour Checkpoint ?</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Close</button>
                                <a href="{{ url('admin/privatetour/'.$detail->id.'/checkpoint/'.$list->id.'/destroy') }}" name="delete_item">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.assign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/tour/tourlist/{detailid}/assign')}}',
                type: "get",
        ...
        $('.unassign').click(function(){
            ...
            l.start();
            $.ajax({
                url: '{{url('api/cms/tour/tourlist/{detailid}/unassign')}}',
                type: "get",
                data: {'id': user_id,'X-CSRF-Token' : $('meta[name=_token]').attr('content')},
                success: function(data){
                    l.stop();
