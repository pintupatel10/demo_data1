...
<div class="col-md-10 col-lg-10 col-sm-10 padding-0">
    ...
    <div class="col-md-12">
        ...
        <div class="col-md-6 light_blue_color center_align_margin">
            ...
            <br>
            <br>
            <?php $tr_list=explode(',',$group->tour_list)?>
            @foreach($tr_list as $list)
                <?php  $list_query=\App\TourList::where('id',$list)->where('status','active')->first(); ?>
                @if(isset($list_query))
                    @if($list_query->display == "Simplified")
                        <a href="{{url('tour/'.$menuid.'/toursimplelist/'.$list_query->id)}}">{{$list_query->title}}</a><br>
                    @else
                        <a href="{{url('tour/'.$menuid.'/tourfulllist/'.$list_query->id)}}">{{$list_query->title}}</a><br>
                    @endif
                @endif
            @endforeach
                @endif
