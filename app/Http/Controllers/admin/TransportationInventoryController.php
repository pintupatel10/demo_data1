...
use App\TransportationInventory;
...
class TransportationInventoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['menu']="TransportationInventory";
        $data['inventory']=TransportationInventory::orderBy('displayorder','DESC')->Paginate($this->pagination);
        return view('admin.transportation.inventory.index',$data);
    }
    ...
    public function create($detailid,$pricegroupid)
    {
        ...
        $data['name'] = DB::table('transportation_prices')->where('pricegroupid', '=', $pricegroupid)->where('deleted_at',null)->lists('title', 'title');
        return view('admin.transportation.inventory.create',$data);
    }
    public function store(Request $request,$detailid,$pricegroupid)
    {
        ...
        $pro = TransportationInventory::create($inventory);
        ...
        for ($m = 1; $m <= $count4; $m++) {
            ...
            TransportationQuota::create($address3);
        }


        Activity::log('TransportationInventory -'.$pro->title.' [Id = '.$pro->id.'] has been inserted');
    ...
    public function edit($id,$detailid,$pricegroupid)
    {
        ...
        $data['inventory'] = TransportationInventory::findOrFail($pricegroupid);
        $data['name'] = DB::table('transportation_prices')->where('pricegroupid', '=', $detailid)->where('deleted_at',null)->lists('title', 'title');
        $data['modes'] = explode(",",$data['inventory']['title']);
        return view('admin.transportation.inventory.edit',$data);
    }
    ...
    public function update(Request $request,$id,$detailid,$pricegroupid)
    {
        ...
        $inventory = TransportationInventory::findOrFail($pricegroupid);
        $input = $request->all();
        ...
        if (isset($input['remove_id4']) && $input['remove_id4'] != "") {
            ...
            foreach ($remove_image as $val) {
                $address = TransportationQuota::findOrFail($val);
                $address->forceDelete();
            }
        }
        ...
        for ($i = 1; $i <= $count4; $i++) {
            ...
            if (!empty($update_id4[$j])) {
                $address3 = TransportationQuota::findOrFail($update_id4[$j]);
                if (!empty($address3)){
                ...
                TransportationQuota::create($addr);
            }
        }
        $inventory->update($input);

        Activity::log('TransportationInventory -'.$inventory->title.' [Id = '.$inventory->id.'] has been updated');
    ...
    public function destroy($id,$detailid,$pricegroupid)
    {
        ...
        Activity::log('TransportationInventory -'.$inventory->title.' [Id = '.$inventory->id.'] has been deleted');
        return redirect('admin/transportation/'.$id.'/pricegroup/'.$detailid.'/edit');
    }
    ...
    public function update_display_order(Request $request)
    {
        ...
        for($i = 1;$i <= $count1;$i++)
        {
            ...
            $inventory = TransportationInventory::findOrFail($pid1);
            $input['displayorder'] = $disp1;
            $inventory->update($input);
        }
        return redirect('admin/transportation/inventory');
    }
