...
class TicketCheckpointController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['menu']="Ticket checkpoint";
        $data['checkpoint']=Ticketcheckpoint::orderBy('id','ASC')->get();
    ...
    public function create($detailid)
    {
        ...
        return view('admin.ticket.checkpoint.create',$data);
    }
    public function store(Request $request,$detailid)
    {
        $input=$request->all();
        $display_order = Ticketcheckpoint::count();
        if ($display_order >= 0) {
        ...
        $tichtkpoint=Ticketcheckpoint::create($input1);

        Activity::log('TicketCheckpoint -'.$tichtkpoint->title.' [Id = '.$tichtkpoint->id.'] has been inserted');
    ...
    public function edit($id,$detailid)
    {
        ...
        $data['menu']="Ticket checkpoint";
        $data['detail'] = $id;
        $data['checkpoint'] = Ticketcheckpoint::findOrFail($detailid);

        return view('admin.ticket.checkpoint.edit',$data);
    }
    ...
    public function update(Request $request, $id,$detailid)
    {
        ...
        $checkpoint = Ticketcheckpoint::findOrFail($detailid);
        $input = $request->all();
        $input['detailid'] = $id;
        if(!empty($request->image_name))
        {
        ...
        Activity::log('TicketCheckpoint -'.$checkpoint->title.' [Id = '.$checkpoint->id.'] has been Updated');
    ...
    public function destroy($id,$detailid)
    {
        ...
        Activity::log('TicketCheckpoint  -'.$checkpoint->title.' [Id = '.$checkpoint->id.'] has been deleted');
        return redirect('admin/ticket/ticketlist/'.$id.'/edit');
    }
    ...
    public function update_display_order(Request $request)
    {
        ...
        for($i = 1;$i <= $count1;$i++)
        {
            ...
            $checkpoint = Ticketcheckpoint::findOrFail($pid1);
            $input['displayorder'] = $disp1;
            $checkpoint->update($input);
        }
        \Session::flash('success', 'Display order Updated successfully!');
        return redirect('admin/ticket/checkpoint');
    }

    public function assign(Request $request)
    {
        $checkpoint = Ticketcheckpoint::findorFail($request['id']);
